        ...
        <div className="h-40 flex flex-row space-between mr-auto ml-auto px-10" style={{maxWidth: '1050px'}}>
            {prev ? <div className="mr-auto">
                <Link to={prev}>
                    ...
                    <p className={`inline-block pl-10 ${smallerThan617 ? 'text-lg' : smallerThan1000 ? 'text-xl' : 'text-2xl'}`}>{prevText}</p>
                </Link>
            </div> : <Fragment />}

            {next ? <div className="ml-auto">
                <Link to={next}>
                    <p className={`inline-block pr-10 ${smallerThan617 ? 'text-lg' : smallerThan1000 ? 'text-xl' : 'text-2xl'}`}>{nextText}</p>
