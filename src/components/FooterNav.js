...
const FooterNav = props => {
    ...
    var nextAndPrev = next && prev && smallerThan550 ? true : false;
        ...
        <div className="h-40 flex flex-row space-between mr-auto ml-auto px-10" style={{maxWidth: '1050px'}}>
            {prev ? <div className="mr-auto">
                <Link to={prev} className={`${nextAndPrev ? 'flex flex-row' : ''}`}>
                    ...
                    <p className={`inline-block ${nextAndPrev || smallerThan550 ? 'pl-3.5 pr-3.5' : 'pl-10'} ${smallerThan420 ? 'text-base' : smallerThan617 ? 'text-lg' : smallerThan1000 ? 'text-xl' : 'text-2xl'}`}>{prevText}</p>
                </Link>
            </div> : <Fragment />}

            {next ? <div className="ml-auto">
                <Link to={next} className={`${nextAndPrev ? 'flex flex-row' : ''}`}>
                    <p className={`inline-block ${nextAndPrev || smallerThan550 ? 'pr-3.5 pl-3.5' : 'pr-10'} ${smallerThan420 ? 'text-base' : smallerThan617 ? 'text-lg' : smallerThan1000 ? 'text-xl' : 'text-2xl'}`}>{nextText}</p>
